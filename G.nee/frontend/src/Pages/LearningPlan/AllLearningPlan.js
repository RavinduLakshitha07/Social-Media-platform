...
function AllLearningPlan() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = posts.filter(
      (post) =>
        ...
        (post.postOwnerName && post.postOwnerName.toLowerCase().includes(query))
    );
  ...
  const handleBuy = (id) => {
    // Navigate to the card selection page with the plan ID
    navigate(`/select-card/${id}`);
  };
        ...
        <div className='continSection' style={{ 
          ...
          <div className='searchinput' style={{ 
            ...
            <input
              ...
              placeholder="Search plans by title, description, category, or creator"
            ...
            {!filteredPosts || filteredPosts.length === 0 ? (
              <div className='not_found_box' style={{
                ...
                <p className='not_found_msg' style={{ color: '#555', fontSize: '18px', margin: '20px 0' }}>
                  {searchOwnerName ? 'No matching learning plans found.' : 'No learning plans available. Be the first to create one!'}
                </p>
                <button 
